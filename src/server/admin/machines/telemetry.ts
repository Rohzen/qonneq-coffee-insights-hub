...
import { getCimbaliTelemetry, getCimbaliAlerts, getCimbaliMachineInfo, CimbaliCredentials } from '../../lib/cimbali';
import { buildWeatherIndex, getDateKeyAndHour, getHistoricalWeather, getWeatherDescription, getWeatherIconKey, parseHourlyData } from '../../lib/weather';
    ...
    try {
        ...
        for (const record of telemetryData) {
            if (record.raw_counters) {
                for (const [key, val] of Object.entries(record.raw_counters)) {
                    debugRawCounters[key] = val as number;
                }
            }
        }
        ...
        return res.status(200).json({
            ...
            data: {
                ...
                alerts,
                weather: weatherData,
                debugFirstRecord: telemetryData.length > 0 ? telemetryData[0] : null,
