...
export async function getBritaResources(creds: BritaCredentials, maxPageSize?: number, continuationToken?: string) {
    ...
    try {
        ...
        const resources = values.map((item: any) => normalizeBritaResource(item));
...
export function normalizeBritaResource(item: any) {
    ...
    if (type === 'PURITY_C_IQ') {
        // API v1 sometimes returns flattened structure where data is directly on resource
    ...
    return {
        ...
        longitude: longitude,
        location: location,
        // Preserve original raw data for detailed views
        raw: item
    };
}
